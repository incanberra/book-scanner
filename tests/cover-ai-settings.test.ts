...
import { aiEndpoint, configureCoverAi, readCoverAi } from '../src/cover-ai';
const endpoint = 'https://example.netlify.app/.netlify/functions/read-cover';
...
it('requires unlocking without sending any image', async () => {
  configureCoverAi('', '');
  await expect(readCoverAi(new Blob(), new AbortController().signal, () => {})).rejects.toThrow('Unlock');
});
